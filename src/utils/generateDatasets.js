...
function convertToJSONL(responseString) {
    const jsonlData = responseString
...
function splitTextIntoChunks(text, tokensPerChunk = 1000) {
    const encoded = (0, gpt_3_encoder_1.encode)(text); // Encode the text into tokens
    const chunks = [];
...
function generateDataForChunk(client_1, textChunk_1, summary_1) {
    return __awaiter(this, arguments, void 0, function* (client, textChunk, summary, maxIterations = 1) {
        ...
        while (remainingText.trim() && iterationCount < maxIterations) {
            const remainingTokens = (0, gpt_3_encoder_1.encode)(remainingText).length;
            console.log("Processing chunk iteration:", iterationCount + 1, "Remaining tokens:", remainingTokens);
            try {
...
function generateDataSetsInChunks(client_1, fileUrl_1) {
    return __awaiter(this, arguments, void 0, function* (client, fileUrl, trainingBreadth = 1000, trainingDepth = 1) {
        const extractedText = yield (0, files_1.extractTextFromFileUrl)(fileUrl); // Extract full document text
        ...
        for (const chunk of textChunks) {
            ...
            const chunkData = yield generateDataForChunk(client, chunk, classification, trainingDepth); // Process each chunk
